...
int srv_execve(int proc_e, const char *exec, size_t exec_len, const char *progname, char *const argv[], char *const env[]);
...
int copy_label(endpoint_t src_e, const char *src_label, size_t src_len, char *dst_label, size_t dst_len);
int init_state_data(endpoint_t src_e, int prepare_state, struct rs_state_data *src_rs_state_data, struct rs_state_data *dst_rs_state_data);
...
#define kill_service(rp, errstr, err) kill_service_debug(__FILE__, __LINE__, rp, errstr, err)
#define crash_service(rp) crash_service_debug(__FILE__, __LINE__, rp)
#define cleanup_service(rp) cleanup_service_debug(__FILE__, __LINE__, rp)
#define cleanup_service_now(rp) do { struct rproc *rpt = (rp); cleanup_service(rpt); cleanup_service(rpt); } while(0)
#define detach_service(rp) detach_service_debug(__FILE__, __LINE__, rp)
...
void request_prepare_update_service_debug(const char *file, int line, struct rproc *rp, int state);
...
#define request_prepare_update_service(rp, state) request_prepare_update_service_debug(__FILE__, __LINE__, rp, state)
#define end_update(result, reply_flag) end_update_debug(__FILE__, __LINE__, result, reply_flag)
