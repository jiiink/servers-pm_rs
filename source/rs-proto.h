...
/* exec.c */
int srv_execve(int proc_e, const char *exec, size_t exec_len, const char *progname,
	char *argv[], char **env);
...
/* manager.c */
int check_call_permission(endpoint_t caller, int call, const struct rproc *rp);
int copy_rs_start(endpoint_t src_e, const char *src_rs_start, struct rs_start *rs_start);
int copy_label(endpoint_t src_e, const char *src_label, size_t src_len, 
...
/* Service management functions with debug info */
int kill_service_debug(const char *file, int line, struct rproc *rp, 
...
#define cleanup_service_now(rp) \
	do { \
		struct rproc *_rpt = (rp); \
		cleanup_service(_rpt); \
		cleanup_service(_rpt); \
	} while(0)
...
void inherit_service_defaults(const struct rproc *def_rp, struct rproc *rp);
void get_service_instances(const struct rproc *rp, struct rproc ***rps, int *length);

/* Executable management */
int read_exec(struct rproc *rp);
void share_exec(const struct rproc *rp_src, struct rproc *rp_dst);
void free_exec(struct rproc *rp);

/* Slot management */
int init_slot(struct rproc *rp, const struct rs_start *rs_start, endpoint_t source);
int edit_slot(struct rproc *rp, const struct rs_start *rs_start, endpoint_t source);
...
/* IPC and privilege management */
char *get_next_label(const char *ptr, char *label, const char *caller_label);
...
/* Update management with debug info */
void request_prepare_update_service_debug(const char *file, int line,
	struct rproc *rp, int state);
void end_update_debug(const char *file, int line, int result, int reply_flag);

/* Update management macros */
...
void rollback_service(struct rproc **src_rpp, struct rproc **dst_rpp);
...
void fill_call_mask(const int *calls, int tot_nr_calls,
	bitchunk_t *call_mask, int call_base, int is_init);

/* String conversion utilities */
#define srv_to_string(RP) srv_to_string_gen((RP), DEBUG)
char* srv_to_string_gen(const struct rproc *rp, int is_verbose);
char* srv_upd_to_string(const struct rprocupd *rpupd);
...
/* Scheduling and management utilities */
