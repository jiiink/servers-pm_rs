...
/* exec.c */
int srv_execve(int proc_e, char *exec, size_t exec_len, char *progname, char *argv[], char **env);
...
int copy_rs_start(endpoint_t src_e, char *src_rs_start, struct rs_start *rs_start);
...
/* safer debug wrappers */
static inline int kill_service(struct rproc *rp, const char *errstr, int err) {
    return kill_service_debug(__FILE__, __LINE__, rp, errstr, err);
}
static inline int crash_service(struct rproc *rp) {
    return crash_service_debug(__FILE__, __LINE__, rp);
}
static inline void cleanup_service(struct rproc *rp) {
    cleanup_service_debug(__FILE__, __LINE__, rp);
}
static inline void cleanup_service_now(struct rproc *rp) {
    struct rproc *rpt = rp;
    cleanup_service(rpt);
    cleanup_service(rpt);
}
static inline void detach_service(struct rproc *rp) {
...
/* safer debug wrappers */
static inline void request_prepare_update_service(struct rproc *rp, int state) {
    request_prepare_update_service_debug(__FILE__, __LINE__, rp, state);
}
static inline void end_update(int result, int reply_flag) {
...
/* safer debug wrapper */
static inline char *srv_to_string(struct rproc *rp) {
    return srv_to_string_gen(rp, DEBUG);
}
