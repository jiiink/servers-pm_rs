...
int do_getepinfo(void)
{
  struct mproc *rmp;
  endpoint_t ep;
  int r, slot, ngroups, req_ngroups;
  ...
  req_ngroups = m_in.m_lsys_pm_getepinfo.ngroups;
  if (req_ngroups < 0) req_ngroups = 0;
  if (ngroups > req_ngroups) ngroups = req_ngroups;

  if (ngroups > 0) {
	size_t bytes = (size_t)ngroups * sizeof(gid_t);
	r = sys_datacopy(SELF, (vir_bytes)rmp->mp_sgroups, who_e,
	    m_in.m_lsys_pm_getepinfo.groups, bytes);
	if (r != OK) return r;
  }
  return rmp->mp_pid;
}
...
#define MAX_LOCAL_PARAMS 2
  ...
  case PMGETPARAM: {
      ...
      is_set = (req == PMSETPARAM || req == OPMSETPARAM);
