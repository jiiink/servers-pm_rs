...
void rupdate_add_upd(struct rprocupd* rpupd)
{
  ...
  struct rprocupd *prev_rpupd = NULL;
  struct rprocupd *walk_rpupd = NULL;
  endpoint_t ep;
  int lu_flags;
  ...
  struct rproc *rp, int state)
{
  ...
  no_reply = (rp->r_flags & RS_PREPARE_DONE) == 0;
...
struct rprocupd* start_update_prepare_next(void)
{
  ...
  while(rpupd != NULL) {
      ...
      if(rupdate.curr_rpupd->next_rpupd == NULL) {
...
int start_update(void)
{
  ...
  if(RUPDATE_IS_UPD_VM_MULTI()) {
      ...
      do_init_ready(&m);
      /* If initialization was successful, complete the update. */
      if(r == OK) {
...
static void end_update_prepare_done(struct rprocupd *rpupd, int result)
{
  ...
  assert(result != OK);
  assert((rpupd->rp->r_flags & RS_INITIALIZING) == 0);
...
static void end_update_initializing(struct rprocupd *rpupd, int result)
{
  /* The service is initializing after a live update. Cleanup the version that
    ...
    struct rprocupd *skip_rpupd, struct rprocupd *only_rpupd)
{
  ...
  struct rprocupd *rpupd = NULL;
  short is_curr;
  short is_before_curr;
  short is_after_curr = 1;

  RUPDATE_REV_ITER(rupdate.last_rpupd, prev_rpupd, rpupd,
      is_curr = (rupdate.curr_rpupd == rpupd) ? 1 : 0;
      is_after_curr = (short)(is_after_curr && !is_curr);
      if(!UPD_IS_PREPARING_ONLY(rpupd)) {
          short is_before_prepare;
          short is_prepare_done;
          short is_initializing;
          is_before_curr = (short)(!is_curr && !is_after_curr);
          ...
          if((skip_rpupd == NULL || rpupd != skip_rpupd) && 
             (only_rpupd == NULL || rpupd == only_rpupd)) {
              ...
              else if(is_before_prepare != 0) {
                  end_update_before_prepare(rpupd, result);
              }
              else if(is_prepare_done != 0) {
