...
int init_service(struct rproc *rp, int type, int flags)
{
  ...
  if((rp->r_priv.s_flags & ROOT_SYS_PROC) != 0) {
      return OK;
  }
  ...
  else if(rp->r_prev_rp != NULL) {
      old_endpoint = rp->r_prev_rp->r_pub->endpoint;
  }

  /* Check flags. */
  if((rp->r_pub->sys_flags & SF_USE_SCRIPT) != 0) {
...
char* srv_to_string_gen(struct rproc *rp, int is_verbose)
{
  ...
  int slot_nr;
  char *srv_string;
  static char srv_string_pool[SRV_STRING_POOL_SIZE][RS_MAX_LABEL_LEN + MAX_STR_LEN];
  ...
  rpub = rp->r_pub;
  slot_nr = (int)(rp - rproc);
  srv_string = srv_string_pool[srv_string_pool_index];
  srv_string_pool_index = (srv_string_pool_index + 1) % SRV_STRING_POOL_SIZE;

#define srv_str(cmd) ((cmd) == NULL || (cmd)[0] == '\0' ? "_" : (cmd))
#define srv_active_str(rp) (((rp)->r_flags & RS_ACTIVE) != 0 ? "*" : " ")
#define srv_version_str(rp) ((rp)->r_new_rp != NULL || (rp)->r_next_rp != NULL ? "-" : \
    ((rp)->r_old_rp != NULL || (rp)->r_prev_rp != NULL ? "+" : " "))

  if(is_verbose != 0) {
      snprintf(srv_string, RS_MAX_LABEL_LEN + MAX_STR_LEN,
		"service '%s'%s%s"
...
{
   static char srv_upd_string[MAX_STR_LEN];
   struct rprocpub *rpub;
   struct rprocpub *next_rpub;
   struct rprocpub *prev_rpub;
   
   if(rpupd == NULL) {
       return "NULL";
   }
   
   rpub = (rpupd->rp != NULL) ? rpupd->rp->r_pub : NULL;
   next_rpub = (rpupd->next_rpupd != NULL && rpupd->next_rpupd->rp != NULL) ?
               rpupd->next_rpupd->rp->r_pub : NULL;
   prev_rpub = (rpupd->prev_rpupd != NULL && rpupd->prev_rpupd->rp != NULL) ?
               rpupd->prev_rpupd->rp->r_pub : NULL;

#define srv_ep(RPUB) ((RPUB) != NULL ? (RPUB)->endpoint : -1)
#define srv_upd_luflag_c(F) ((rpupd->lu_flags & (F)) != 0 ? '1' : '0')
#define srv_upd_iflag_c(F) ((rpupd->init_flags & (F)) != 0 ? '1' : '0')

   snprintf(srv_upd_string, sizeof(srv_upd_string),
 ...
 */
  ...
  s = sys_statectl(SYS_STATE_ADD_IPC_WL_FILTER, ipc_filter, sizeof(ipc_filter));
  if(s != OK) {
      panic("RS: rs_receive_ticks: setting IPC filter failed: %d", s);
  }

  /* Set a new alarm, and get information about the previous alarm. */
  s = sys_setalarm2(ticks, FALSE, &time_left, &uptime);
  if(s != OK) {
  ...
  s = sys_statectl(SYS_STATE_CLEAR_IPC_FILTERS, NULL, 0);
  if(s != OK) {
      panic("RS: rs_receive_ticks: clearing IPC filter failed: %d", s);
  }
...
int sched_init_proc(struct rproc *rp)
{
  ...
  is_usr_proc = (rp->r_priv.s_flags & SYS_PROC) == 0;
  if(is_usr_proc != 0) {
      assert(rp->r_scheduler == NONE);
  } else {
      assert(rp->r_scheduler != NONE);
  }
...
void rs_idle_period(void)
{
  ...
  for (rp=BEG_RPROC_ADDR; rp<END_RPROC_ADDR; rp++) {
      ...
      if((rp->r_flags & RS_ACTIVE) != 0 && 
         (rpub->sys_flags & SF_USE_REPL) != 0 && 
         rp->r_next_rp == NULL) {
          if(rpub->endpoint == VM_PROC_NR && 
             (rp->r_old_rp != NULL || rp->r_new_rp != NULL)) {
...
#define rupdate_flag_c(F) ((rupdate.flags & (F)) != 0 ? '1' : '0')
  ...
  printf("A %s-component update is %s, flags(UIRV)=%c%c%c%c:\n", 
      ...
      (rupdate.rs_rpupd != NULL) ? '1' : '0', 
      (rupdate.vm_rpupd != NULL) ? '1' : '0');
  PRINT_SEP();
