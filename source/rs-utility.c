...
int init_service(struct rproc *rp, int type, int flags)
{
  ...
  } else if (rp->r_prev_rp != NULL && rp->r_prev_rp->r_pub != NULL) {
      old_endpoint = rp->r_prev_rp->r_pub->endpoint;
  }

  /* Check flags. */
  if (rp->r_pub != NULL && (rp->r_pub->sys_flags & SF_USE_SCRIPT) != 0) {
...
/* Fill a call mask from an unordered set of calls. */
  ...
  if (nr_calls == 1 && calls[0] == ALL_C) {
      ...
      for (i = 0; i < nr_calls; i++) {
          if ((calls[i] - call_base) >= 0 && (calls[i] - call_base) < tot_nr_calls) {
...
{
   static char srv_upd_string[512]; /* Increased buffer size for safety */
   struct rprocpub *rpub, *next_rpub, *prev_rpub;

   if (rpupd == NULL) {
       strlcpy(srv_upd_string, "INVALID_UPDATE", sizeof(srv_upd_string));
...
#define srv_upd_iflag_c(F) (((rpupd->init_flags & F) != 0) ? '1' : '0')
   ...
   snprintf(srv_upd_string, sizeof(srv_upd_string), 
            ...
            (rpupd->prepare_state_data.eval_addr != NULL) ? rpupd->prepare_state_data.eval_addr : "",
...
void rs_idle_period(void)
{
  ...
  for (rp = BEG_RPROC_ADDR; rp < END_RPROC_ADDR; rp++) {
      ...
      if ((rp->r_flags & RS_ACTIVE) != 0 && 
          (rpub->sys_flags & SF_USE_REPL) != 0 && rp->r_next_rp == NULL) {
