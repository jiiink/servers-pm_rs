...
/* Maximum size for ptrace data transfer */
#define PTRACE_MAX_SIZE	4096
...
do_trace(void)
{
  ...
  case T_ATTACH:	/* attach to an existing process */
	if ((child = find_proc(pid)) == NULL) return ESRCH;
  ...
  case T_SETRANGE:	/* get/set range of values */
	...
	if (pr.pr_size == 0 || pr.pr_size > PTRACE_MAX_SIZE) return EINVAL;
...
/* A traced process got a signal so stop it. */
  ...
  if (!rmp) {
      printf("PM: trace_stop called with NULL pointer\n");
      return;
  }

  if (rmp->mp_tracer < 0 || rmp->mp_tracer >= NR_PROCS) {
      printf("PM: trace_stop: invalid tracer %d\n", rmp->mp_tracer);
      return;
  }
