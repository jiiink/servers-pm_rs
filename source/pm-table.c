...
int (* const call_vec[NR_PM_CALLS])(void) = {
	...
	CALL(PM_SETEUID)	= do_set,		/* setuid/seteuid(2) */
	CALL(PM_SETEGID)	= do_set,		/* setgid/setegid(2) */
	...
	CALL(PM_EXEC_NEW)	= do_newexec,		/* PM-internal for VFS/RS */
	CALL(PM_EXEC_RESTART)	= do_execrestart,	/* PM-internal for RS */
